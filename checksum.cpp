/*	Quick and dirty program for fixing CRC checksums in AC: Happy Home Designer save file (takumi.dat)
 *	The program runs directly on the computer, not on the 3DS
 * 
 *	Copyright (C) 2026 IC
 *	Copyright (C) 2020 RedShyGuy, SuperSaiyajinStackZ
 */
/*
*   This file is based on /core/source/utils/checksum.cpp from Happy-Home-Editor (https://github.com/SuperSaiyajinStackZ/Happy-Home-Editor)
*   Copyright (C) 2020 RedShyGuy, SuperSaiyajinStackZ
*
...
uint32_t CalculateCRC32Reflected(uint8_t *buf, uint32_t size) {
...
/* Verify the CRC32. */
uint32_t VerifyCRC32(uint32_t crc, uint8_t *buf, uint32_t startOffset, uint32_t size) {
...
/* Update the CRC32. */
uint32_t UpdateCRC32(uint8_t *rawData, uint32_t startOffset, uint32_t size) {
	uint32_t crc32 = CalculateCRC32Reflected(rawData + startOffset + 4, size);
	
	uint8_t currcrc[4];
	
	for(uint8_t i=0;i<4;i++) currcrc[i] = rawData[startOffset+i];
	
	printf("Offset: 0x%x, current CRC: %x %x %x %x\n", startOffset, currcrc[0], currcrc[1], currcrc[2], currcrc[3]);
	
	uint64_t off;
	brstm_encoder_writebytes(rawData,brstm_encoder_getByteUint(crc32,4,0),4,off=startOffset);
	
	bool difference = 0;
	
	for(uint8_t i=0;i<4;i++) {
		if(currcrc[i] != rawData[startOffset+i]) {difference = 1; changed = 1; break;}
	}
	
	printf("Offset: 0x%x, new CRC: %x %x %x %x\n\n", startOffset, rawData[startOffset+0], rawData[startOffset+1], rawData[startOffset+2], rawData[startOffset+3] );
	
	//SaveUtils::Write<uint32_t>(rawData, startOffset, crc32); // write calculated crc32.
	return crc32;
}

/* Fix the CRC32 for AC:HHD. */
void FixCRC32s(uint8_t *data) {
    UpdateCRC32(data, 0x1A0, 0x1C);
    UpdateCRC32(data, 0x1C0, 0x2BE75C);
    UpdateCRC32(data, 0x2BE920, 0xC);
}

int main(int argc, char** argv) {
	if(argc != 2) {
		printf("Usage: %s [filename / path to takumi.dat file]\n", argv[0]);
		printf("Fixed file will be saved with \".CRCFIXED\" extension\n");
		printf("This program is free software, see the LICENSE file for more information.\nThis program comes with NO WARRANTY, use at your own risk.\n");
		return 255;
	}
	...
	char* ofilename;
	ofilename = new char[strlen(argv[1])+32];
	strcpy(ofilename,argv[1]);
	strcat(ofilename, ".CRCFIXED");
